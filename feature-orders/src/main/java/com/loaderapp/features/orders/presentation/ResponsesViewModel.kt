...
internal fun List<OrderUiModel>.toResponsesItems(availability: Map<String, ResponderAvailability>): List<OrderResponsesUiModel> =
    asSequence()
        .filter { it.order.status == OrderStatus.STAFFING }
        .map { order ->
            val responses =
                order.visibleApplicants.map { application ->
                    ...
                    val isBusyOutsideCurrentOrder =
                        loaderAvailability?.isBusy == true &&
