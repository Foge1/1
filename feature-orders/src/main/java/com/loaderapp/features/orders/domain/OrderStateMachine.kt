    ...
    constructor(
        ...
        private fun canWithdraw(
            ...
            val hasActiveApplication =
                order.applications.any {
                    it.loaderId == actor.id && it.status in
                        setOf(
            ...
            return if (hasActiveApplication) {
                Decision.allowed()
            } else {
                Decision.denied(OrderActionBlockReason.NoActiveApplicationToWithdraw)
            }
        }
        ...
        private fun canSelect(
            ...
            if (order.status != OrderStatus.STAFFING) {
                return Decision.denied(
                    OrderActionBlockReason.UnsupportedEventForStatus(OrderEvent.SELECT, order.status),
                )
            }
        ...
        private fun canUnselect(
            order: Order,
            actor: CurrentUser,
        ): Decision {
            if (order.status != OrderStatus.STAFFING) {
                return Decision.denied(
                    OrderActionBlockReason.UnsupportedEventForStatus(OrderEvent.UNSELECT, order.status),
                )
            }
            if (actor.role != Role.DISPATCHER || actor.id != order.createdByUserId) {
                return Decision.denied(OrderActionBlockReason.OnlyDispatcherCreatorCanManageStaffing)
        ...
        private fun canCancel(
            ...
            if (order.status !in setOf(OrderStatus.STAFFING, OrderStatus.IN_PROGRESS)) {
                return Decision.denied(
                    OrderActionBlockReason.UnsupportedEventForStatus(OrderEvent.CANCEL, order.status),
                )
            }
            if (actor.role != Role.DISPATCHER || actor.id != order.createdByUserId) {
        ...
        private fun canComplete(
            ...
            return if (allowed) {
                ...
                Decision.denied(OrderActionBlockReason.OnlyDispatcherCreatorOrAssignedLoaderCanComplete)
            }
        }

        private fun canExpire(order: Order): Decision {
            return if (order.status == OrderStatus.STAFFING) {
                Decision.allowed()
            } else {
                Decision.denied(OrderActionBlockReason.UnsupportedEventForStatus(OrderEvent.EXPIRE, order.status))
            }
        }

        private fun canOpenChat(
            order: Order,
            actor: CurrentUser,
            context: OrderRulesContext,
        ): Boolean =
