...
class SelectUnselectApplicantUseCaseTest {
    @Test
    fun `select and unselect call repository and update order state`() =
        runBlocking {
    ...
    @Test
    fun `select returns AlreadyAssigned and does not mutate repository when loader is busy`() =
        runBlocking {
    ...
    private class TrackingOrdersRepository(
        ...
        fun currentApplicationStatus(): OrderApplicationStatus = state.value.first().applications.first().status
