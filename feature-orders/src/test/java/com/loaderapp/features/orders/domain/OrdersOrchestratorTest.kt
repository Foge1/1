...
class OrdersOrchestratorTest {
    ...
    @Test
    fun `Apply succeeds when order is STAFFING and no active assignment`() =
        runBlocking {
            val repo =
    ...
    @Test
    fun `Apply fails when loader already has an active assignment`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
                    orders = listOf(testOrder(id = 1L, status = OrderStatus.STAFFING)),
    ...
    @Test
    fun `Apply fails when loader has reached applied count limit`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
                    orders = listOf(testOrder(id = 1L, status = OrderStatus.STAFFING)),
    ...
    @Test
    fun `Apply fails when order is not STAFFING`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
    ...
    @Test
    fun `Apply fails when actor is dispatcher`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
                    orders = listOf(testOrder(id = 1L, status = OrderStatus.STAFFING)),
    ...
    @Test
    fun `Select succeeds when dispatcher is creator`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
    ...
    @Test
    fun `Select fails when dispatcher is not creator`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
    ...
    @Test
    fun `Unselect fails when dispatcher is not creator`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
                    orders = listOf(testOrder(id = 1L, status = OrderStatus.STAFFING, createdBy = "other-dispatcher")),
    ...
    @Test
    fun `Start succeeds when dispatcher is creator and selectedCount equals workersTotal`() =
        runBlocking {
            val selected =
                listOf(
                    OrderApplication(
    ...
    @Test
    fun `Start fails when selectedCount less than workersTotal`() =
        runBlocking {
            val selected =
                listOf(
                    OrderApplication(
    ...
    @Test
    fun `Start fails when dispatcher is not creator`() =
        runBlocking {
            val selected =
                listOf(
                    OrderApplication(
    ...
    @Test
    fun `Cancel fails for non-creator in STAFFING`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
    ...
    @Test
    fun `Cancel succeeds for creator in STAFFING`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
                    orders = listOf(testOrder(id = 1L, status = OrderStatus.STAFFING, createdBy = DISPATCHER_ID)),
    ...
    fun `Refresh returns Success`() =
        runBlocking {
            val result =
                buildOrchestrator(InMemoryOrdersRepository(), loaderUser)
                    .execute(OrdersCommand.Refresh)
            assertTrue(result is UseCaseResult.Success)
        }

    @Test
    fun `Create returns Success for valid draft`() =
        runBlocking {
            val result =
                buildOrchestrator(InMemoryOrdersRepository(), dispatcherUser)
