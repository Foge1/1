...
class OrdersOwnershipUseCasesTest {
    ...
    fun `select succeeds for dispatcher creator`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(listOf(baseOrder(id = 1, createdBy = "d-1", status = OrderStatus.STAFFING)))
            val useCase =
    ...
    fun `select fails for non-creator dispatcher`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
                    listOf(baseOrder(id = 1, createdBy = "d-other", status = OrderStatus.STAFFING)),
                )
            val useCase =
                SelectApplicantUseCase(
                    repo,
    ...
    fun `observe dispatcher returns only own created orders`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(listOf(baseOrder(id = 1, createdBy = "1"), baseOrder(id = 2, createdBy = "2")))
            val useCase =
                ObserveOrdersForRoleUseCase(repo, StaticCurrentUserProvider(CurrentUser("1", Role.DISPATCHER)))
