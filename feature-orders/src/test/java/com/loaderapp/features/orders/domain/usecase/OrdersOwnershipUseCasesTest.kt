...
class OrdersOwnershipUseCasesTest {
    ...
    @Test
    fun `createOrder sets createdBy and status STAFFING`() =
        runBlocking {
    ...
    @Test
    fun `apply succeeds for loader on STAFFING order with no active assignment`() =
        runBlocking {
            val repo = InMemoryOrdersRepository(listOf(baseOrder(id = 1, status = OrderStatus.STAFFING)))
            val useCase =
                ApplyToOrderUseCase(
                    repo,
                    StaticCurrentUserProvider(CurrentUser("loader-1", Role.LOADER)),
                    OrderStateMachine(OrdersLimits()),
                )
    ...
    @Test
    fun `apply fails for loader with active assignment`() =
        runBlocking {
            val repo =
    ...
    @Test
    fun `apply fails when loader has 3 active applications`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
                    orders = listOf(baseOrder(id = 1, status = OrderStatus.STAFFING)),
    ...
    @Test
    fun `apply fails for dispatcher`() =
        runBlocking {
            val repo = InMemoryOrdersRepository(listOf(baseOrder(id = 1, status = OrderStatus.STAFFING)))
            val useCase =
                ApplyToOrderUseCase(repo, StaticCurrentUserProvider(CurrentUser("d-1", Role.DISPATCHER)), OrderStateMachine(OrdersLimits()))
    ...
    @Test
    fun `select succeeds for dispatcher creator`() =
        runBlocking {
            val repo = InMemoryOrdersRepository(listOf(baseOrder(id = 1, createdBy = "d-1", status = OrderStatus.STAFFING)))
            val useCase =
                SelectApplicantUseCase(
                    repo,
                    StaticCurrentUserProvider(CurrentUser("d-1", Role.DISPATCHER)),
                    OrderStateMachine(OrdersLimits()),
                )
    ...
    @Test
    fun `select fails for non-creator dispatcher`() =
        runBlocking {
    ...
    @Test
    fun `unselect fails for loader`() =
        runBlocking {
            val repo = InMemoryOrdersRepository(listOf(baseOrder(id = 1, createdBy = "d-1", status = OrderStatus.STAFFING)))
            val useCase =
                UnselectApplicantUseCase(
                    repo,
                    StaticCurrentUserProvider(CurrentUser("loader-1", Role.LOADER)),
                    OrderStateMachine(OrdersLimits()),
                )
    ...
    @Test
    fun `start succeeds when creator and selectedCount equals workersTotal`() =
        runBlocking {
            val apps =
                listOf(
            ...
            val useCase =
                StartOrderUseCase(repo, StaticCurrentUserProvider(CurrentUser("d-1", Role.DISPATCHER)), OrderStateMachine(OrdersLimits()))
    ...
    @Test
    fun `start fails when selectedCount does not match workersTotal`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
                    listOf(
                        baseOrder(id = 1, createdBy = "d-1", status = OrderStatus.STAFFING, workersTotal = 2, applications = emptyList()),
                    ),
                )
            val useCase =
                StartOrderUseCase(repo, StaticCurrentUserProvider(CurrentUser("d-1", Role.DISPATCHER)), OrderStateMachine(OrdersLimits()))
    ...
    @Test
    fun `start fails when actor is loader`() =
        runBlocking {
            val apps =
                listOf(
                    OrderApplication(orderId = 1, loaderId = "l1", status = OrderApplicationStatus.SELECTED, appliedAtMillis = 0L),
            ...
            val useCase =
                StartOrderUseCase(repo, StaticCurrentUserProvider(CurrentUser("loader-1", Role.LOADER)), OrderStateMachine(OrdersLimits()))
    ...
    @Test
    fun `observe dispatcher returns only own created orders`() =
        runBlocking {
    ...
    @Test
    fun `observe loader returns all staffing and own in progress orders`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
                    listOf(
    ...
    @Test
    fun `observe orders reacts to current user changes`() =
        runBlocking {
            val repo =
                InMemoryOrdersRepository(
                    listOf(
