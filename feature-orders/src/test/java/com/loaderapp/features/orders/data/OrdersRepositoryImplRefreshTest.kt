...
class OrdersRepositoryImplRefreshTest {
    @Test
    fun `refresh does not expire soon orders`() =
        runBlocking {
    ...
    @Test
    fun `refresh expires exact orders older than threshold`() =
        runBlocking {
            val repository = FakeOrdersRepository()
            repository.createOrder(
                Order(
    ...
    @Test
    fun `InMemoryOrdersDao expireStaffingExactOrders updates only matching rows`() =
        runBlocking {
    ...
    @Test
    fun `InMemoryAssignmentsDao countAssignmentsByLoaderAndStatus works`() =
        runBlocking {
    ...
    @Test
    fun `InMemoryApplicationsDao updateApplicationsStatusByOrder rejects APPLIED`() =
        runBlocking {
            val dao = InMemoryApplicationsDao()
            dao.upsertApplication(
                OrderApplicationEntity(1L, "loader-a", OrderApplicationStatus.APPLIED.name, 100L, null),
            )
            dao.upsertApplication(
                OrderApplicationEntity(1L, "loader-b", OrderApplicationStatus.SELECTED.name, 101L, null),
            )
            dao.updateApplicationsStatusByOrder(
                1L,
                OrderApplicationStatus.APPLIED.name,
                OrderApplicationStatus.REJECTED.name,
            )
    ...
    @Test
    fun `InMemoryApplicationsDao countActiveApplicationsForLimit ignores closed orders`() =
        runBlocking {
            val ordersDao =
                InMemoryOrdersDao().apply {
            ...
            applicationsDao.upsertApplication(
                OrderApplicationEntity(1L, "loader-1", OrderApplicationStatus.APPLIED.name, 1L, null),
            )
            applicationsDao.upsertApplication(
                OrderApplicationEntity(2L, "loader-1", OrderApplicationStatus.SELECTED.name, 2L, null),
            )
            applicationsDao.upsertApplication(
                OrderApplicationEntity(3L, "loader-1", OrderApplicationStatus.APPLIED.name, 3L, null),
            )
            applicationsDao.upsertApplication(
                OrderApplicationEntity(4L, "loader-1", OrderApplicationStatus.APPLIED.name, 4L, null),
            )
...
internal class InMemoryOrdersDao : OrdersDao {
    ...
    override suspend fun insertOrder(order: OrderEntity): Long {
        ...
        orders.update { current ->
            ...
            if (existing >= 0) {
                current.mapIndexed { i, e -> if (i == existing) order.copy(id = newId) else e }
            } else {
                current + order.copy(id = newId)
            }
        }
        return newId
    }
...
internal class InMemoryApplicationsDao(
    ...
    override suspend fun updateApplicationsStatusByOrder(
        orderId: Long,
        fromStatus: String,
        toStatus: String,
    ) {
    ...
    override suspend fun countApplicationsByLoaderAndStatuses(
        loaderId: String,
        statuses: List<String>,
    ): Int = apps.value.count { it.loaderId == loaderId && it.status in statuses }
...
internal class InMemoryAssignmentsDao : AssignmentsDao {
    ...
    override suspend fun updateAssignmentsStatusByOrder(
        orderId: Long,
        newStatus: String,
    ) {
    ...
    override suspend fun countAssignmentsByLoaderAndStatus(
        loaderId: String,
        status: String,
    ): Int = assigns.value.count { it.loaderId == loaderId && it.status == status }

    override suspend fun findActiveAssignmentsByLoaders(
        ...
        .map {
            com.loaderapp.features.orders.data.local.dao.LoaderOrderPair(
    ...
    override suspend fun countAssignmentsByOrderLoaderAndStatuses(
        orderId: Long,
        loaderId: String,
        statuses: List<String>,
    ): Int = assigns.value.count { it.orderId == orderId && it.loaderId == loaderId && it.status in statuses }
}
