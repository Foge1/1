...
object OrderMapper {
     ...
     */
    fun toDomainList(entities: List<Order>): List<OrderModel> = entities.map { toDomain(it) }

    /**
     * Конвертация списка Domain -> Entity
     */
    fun toEntityList(models: List<OrderModel>): List<Order> = models.map { toEntity(it) }
}
