...
class AuthRepositoryImpl
    ...
    constructor(
                ...
                when (val resolvedUserResult = resolveOrCreateUser(normalizedName, role)) {
                    ...
                    is AppResult.Failure -> {
                        appLogger.breadcrumb(
                            "auth",
                            "login_failed",
                            mapOf("reason" to resolvedUserResult.error::class.simpleName.orEmpty()),
                        )
        ...
        private suspend fun resolveAuthenticatedState(userId: Long): SessionState =
            when (val result = userRepository.getUserById(userId).toAppResult()) {
        ...
        private suspend fun resolveOrCreateUser(
            ...
            when (val existingUserResult = userRepository.getUserByNameAndRole(name, role).toAppResult()) {
                is AppResult.Success -> {
