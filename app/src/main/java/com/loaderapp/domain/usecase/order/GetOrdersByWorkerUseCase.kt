 ...
 */
data class GetOrdersByWorkerParams(
    val workerId: Long,
)
 ...
 */
class GetOrdersByWorkerUseCase
    @Inject
    constructor(
        private val orderRepository: OrderRepository,
    ) : FlowUseCase<GetOrdersByWorkerParams, Flow<List<OrderModel>>>() {
        override fun execute(params: GetOrdersByWorkerParams): Flow<List<OrderModel>> = orderRepository.getOrdersByWorker(params.workerId)
    }
